...
(function() {
	if(BigBlueButtonBN == null)
		BigBlueButtonBN = new Object();
	
	BigBlueButtonBN.joinURL = function(url) {
		window.location = url;
	}

	BigBlueButtonBN.callback = function(url) {
		// Not elegant, but works around a bug in IE8
		if ($("#HeartBeatDIV").text().search("true") > 0) { //isMeeting
			bigbluebuttonbn.joinURL(url);
		}
	}

	BigBlueButtonBN.recordingActionCall = function(action, recordingID) {
